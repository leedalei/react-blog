...
var microMarkdown = {
  ...
  parse: function (str, strict) {
    ...
    /* codeblock */
    while ((stra = microMarkdown.regexobject.codeblock.exec(str)) !== null) {
      microMarkdown.regexobject.codeblock.lastIndex = 0;

      var language = stra[1].toUpperCase(),
          hljsStr = hljs.highlightAuto(stra[2]).value,
          hljsStr_md = '```' + hljsStr + '```';

      crc32str = microMarkdown.crc32(hljsStr_md);
      microMarkdown.codeblocks[crc32str] = '<br/><div data-language="' + language + '" class="language-badge"><pre><code class="hljs">' + hljsStr.replace(/\n/gm, '<br/>') + '</code></pre></div><br/>';
      //str = str.replace(stra[0], '&§§§' + crc32str + '§§§&'); lwc update 20190719 
      str = str.replace(stra[0], '&§§§' + crc32str + '§§§'); 
    }

    /* headlines */
    var currentSecondHeaderPlace = 0;
    while ((stra = microMarkdown.regexobject.headline.exec(str)) !== null) {
      microMarkdown.regexobject.headline.lastIndex = 0;

      var level = stra[1].length;

      stra[2] = stra[2]
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')

      var formatText = stra[2]
        .replace(/[\.\,\，\。\；\!\！]/g, '-') // 标点
        .replace(/\s+/g, '-') // 空格
        .replace(/(^-+)|(-+$)/g, '') // 移除句首句末多余分隔符
        .replace(/`/g, '') // 移除 <code></code>
        .trim() // 移除空格

      var titleText = stra[2]
        .replace(/`/g, '')
        .trim()

      var navText = stra[2].replace(/`/g, ' ')

      if (level > 1) {

        if (level == 2) {
          currentSecondHeaderPlace++;
        } else {
          formatText = `${currentSecondHeaderPlace}-${formatText}`;
        }

        str = str.replace(stra[0], `<h${level} id="${formatText}"><a href="#${formatText}" class="headerLink" title="${titleText}" data-origin="${navText}">${stra[2]}</a></h${level}>`);
      
      } else {
        str = str.replace(stra[0], '<h' + level + '>' + stra[2].trim() + '</h' + level + '>');
      }
    ...
    while ((stra = microMarkdown.regexobject.code.exec(str)) !== null) {
      microMarkdown.regexobject.code.lastIndex = 0;

      crc32str = microMarkdown.crc32(stra[0]);
      microMarkdown.codeblocks[crc32str] = '<code>' + microMarkdown.htmlEncode(stra[1]).replace(/\ /gm, '&nbsp;') + '</code>';
      str = str.replace(stra[0], '$§§§' + crc32str + '§§§$'); 
    }

    /* badge */
    while ((stra = microMarkdown.regexobject.badge.exec(str)) !== null ){
      str = str.replace(stra[0], '<i class="version-badge"><span class="version-badge-left">' + stra[1] + '</span><span class="version-badge-right type-' + stra[3].length + '">' + stra[2] + '</span></i>');
    }
    ...
    while ((stra = microMarkdown.regexobject.lists.exec(str)) !== null) {
      microMarkdown.regexobject.lists.lastIndex = 0;
    ...
    while ((stra = microMarkdown.regexobject.tables.exec(str)) !== null) {
      microMarkdown.regexobject.tables.lastIndex = 0;

      repstr = '<table><tr>';
      helper = stra[1].replace(/^\s*\||\|\s*\n*$/g, '').split('|');
      calign = stra[4].replace(/^\s*\||\|\s*\n*$/g, '').split('|');
      for (i = 0; i < helper.length; i++) {
        if (calign.length <= i) {
          calign.push(0);
    ...
    /* quote */
    while ((stra = microMarkdown.regexobject.quote.exec(str)) !== null) {
      microMarkdown.regexobject.quote.lastIndex = 0;

      var quoteType = stra[2] || 'quote';
      str = str.replace(stra[0], '<p class="' + quoteType + '">' + stra[3].replace(/\n>?/g, '<br/>') + '</p>'); 
    }

    /* links */
    while ((stra = microMarkdown.regexobject.links.exec(str)) !== null) {
      microMarkdown.regexobject.links.lastIndex = 0;

      if (stra[0].substr(0, 1) === '!') {
        str = str.replace(stra[0], '<img src="' + stra[2] + '" alt="' + stra[1] + '" title="' + stra[1] + '" /><a class="img_title" target="_blank" href="' + stra[2] + '">' + stra[1] +'</a>%%%1br%%%');
      } else {
        str = str.replace(stra[0], '<a href="' + stra[2] + '" target="_blank">' + stra[1] + '</a>');
      }
    }
    while ((stra = microMarkdown.regexobject.mail.exec(str)) !== null) {
      microMarkdown.regexobject.mail.lastIndex = 0;

      str = str.replace(stra[0], '<a href="mailto:' + stra[1] + '">' + stra[1] + '</a>');
    }
    while ((stra = microMarkdown.regexobject.url.exec(str)) !== null) {
      microMarkdown.regexobject.url.lastIndex = 0;

      repstr = stra[1];
      if (repstr.indexOf('://') === -1) {
        repstr = 'http://' + repstr;
      }
      str = str.replace(stra[0], '<a href="' + repstr + '" target="_blank">' + repstr.replace(/(https:\/\/|http:\/\/|mailto:|ftp:\/\/)/gmi, '') + '</a>');
    }
    while ((stra = microMarkdown.regexobject.reflinks.exec(str)) !== null) {
      microMarkdown.regexobject.reflinks.lastIndex = 0;

      helper1 = new RegExp('\\[' + stra[2] + '\\]: ?([^ \n]+)', "gi");
      if ((helper = helper1.exec(str)) !== null) {
        str = str.replace(stra[0], '<a href="' + helper[1] + '" target="_blank">' + stra[1] + '</a>').replace(/^\s+|\s+$/g, '');
    ...
    while ((stra = microMarkdown.regexobject.smlinks.exec(str)) !== null) {
      microMarkdown.regexobject.smlinks.lastIndex = 0;
      ...
      str = str.replace(stra[0], '<a href="' + repstr + '" target="_blank">' + stra[1] + '</a>');
    }
    while ((stra = microMarkdown.regexobject.url2.exec(str)) !== null) {
      microMarkdown.regexobject.url2.lastIndex = 0;

      repstr = stra[1]
      str = str.replace(stra[0], '<a href="' + repstr + '" target="_blank">' + repstr + '</a>');
    }

    /* horizontal line */
    while ((stra = microMarkdown.regexobject.hr.exec(str)) !== null) {
      microMarkdown.regexobject.hr.lastIndex = 0;
    ...
    if ((microMarkdown.useajax !== false)) {
      while ((stra = microMarkdown.regexobject.include.exec(str)) !== null) {
        microMarkdown.regexobject.include.lastIndex = 0;
    ...
    for(var index in microMarkdown.codeblocks) { 
      if(microMarkdown.codeblocks.hasOwnProperty(index)) {
        ...
        str = str.replace(reg, microMarkdown.codeblocks[index]); 
      }
    }
    str = str.replace('&#x0024&amp;', '$&');
    // 标题后的代码块不接换行
    str = str.replace(/<\/h(\d)>\s*(<br\/>\s*)+<div data-language/g, function(match, $1, $2) {
      return `</h${$1}><div data-language`;
    })
    
    // ul 列表后的代码块不接换行
    str = str.replace(/<\/ul>\s*(<br\/>\s*)+<div data-language/g, '</ul><div data-language')

    microMarkdown.codeblocks = [];
